function WindyWave(params) {
    ...
    var VELOCITY_SCALE =0.005*(Math.pow(0.45,params.velocityScale));//(params.velocityScale || 0.005) * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
    var MAX_PARTICLE_AGE = params.particleAge || 200; // max number of frames a particle is drawn before regeneration
    var PARTICLE_LINE_WIDTH = params.lineWidth ||10+0.3*params.velocityScale; // line width of a drawn particle
    var PARTICLE_MULTIPLIER = params.particleMultiplier || 1 / 300; // 10-500，1-150particle count scalar (completely arbitrary--this values looks nice)//越大越少，控制粒子密度
    ...
    var defaulColorScale = ["rgb(255,255,255)"];//控制粒子颜色 可以为颜色数组 目前设置为单一颜色，后续修改
...
//数据解析
    var createBuilder = function createBuilder(data) {//数据解析
        ...
        data.forEach(function (record) {
            ...
            switch (record.header.parameterName) {
                case "UGRD_WIND":
                    uComp = record; break;
                case "VGRD_WIND":
                    vComp = record; break;
  ...
  */
   ...
   */
    ...
    var animate = function animate(bounds, field) {
        ...
        var fadeFillStyle = "rgba(0, 0, 0, 0.7)";
